...
/** Request timeout — prevents frozen scanner UI */
...
/** Paths where 401/403 should NOT trigger auto-logout (health checks) */
...
export async function gateApi(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      return await gateApiFetch(path, options);
    } catch (err) {
      lastError = err;
      // Don't retry auth failures or timeouts — they won't succeed on retry
...
async function gateApiFetch(
  ...
  try {
    ...
    // Auto-logout on auth failure — but NOT for auth endpoints or health checks
