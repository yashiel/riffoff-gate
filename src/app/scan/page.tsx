...
export default function ScanPage() {
  ...
  const { stats: sseStats, connectionState } = useGateSSEContext();
  useWakeLock();

  // Sync SSE stats into local state (SSE provides authoritative server counts)
  useEffect(() => {
    if (sseStats) {
      setCheckedIn(sseStats.total.checkedIn);
      setTotal(sseStats.total.totalTickets);
    }
  }, [sseStats]);
  ...
  return (
    <>
      {/* Status bar — prefer SSE connection state over ping-based connectivity */}
      <StatusBar
        status={
          connectionState === "connected"
            ? connectivity // SSE connected: use ping-based for degraded detection
